...
| 1   | Re-anchor hot path O(changed): linear findTurns dedup, shared per-pass turns+text cache, self-mutation filter, dirty-turn search ordering (never a filter), isConnected orphan probe, single scroll entry (+ignore extension-internal scrolls, passive listeners)                 | pending     | reanchor measure ≈0 on stable frames; O(changed) during streams  |
| 2   | Startup proportional to visible: sweepDrafts getKeys (guarded), shared-cache restore, lazy message render (chips none; expanded sync-until-cap + idle fill, flush on scroll-up/refresh/destroy)                                                                                   | pending     | load marks O(T + visible); no perceptible load stall             |
